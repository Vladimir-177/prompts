...
2. *(New)* **Query-Intent Classification:** Before querying MCP servers, the agent classifies the query intent (e.g., legal, how-to, account-specific, troubleshooting) to inform how it weights results from each source.
3. The Root Agent interrogates all 4 MCP servers in parallel to fetch top-K relevant chunks from each topic.
4. *(New)* **Chunk Payload Specification:** Each chunk returned by an MCP server must include: `chunk_id`, `text`, `Vector_Score`, `source_mcp`, and `metadata` (e.g., document title, last updated date).
...
7. *(New)* **Clarification Protocol:** If the query is ambiguous and could match multiple domains equally, the agent should ask a clarifying question before generating a speculative answer.
8. *(New)* **Confidence Indicator:** The agent should output a confidence level (High / Medium / Low) alongside each answer to signal how well-supported it is by the retrieved context.